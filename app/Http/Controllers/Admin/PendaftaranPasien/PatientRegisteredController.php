...
class PatientRegisteredController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = 'SELECT DISTINCT prd.kode_daftar, pr.title as tipe, p.name as pasien , dep.title as spesialis, depd.name as dokter, ds.days,ds.start,ds.end FROM `patient_registration_data` as prd JOIN patient_registrations as pr ON prd.patient_registration_id = pr.id JOIN patients as p ON prd.patient_id = p.id JOIN departments as dep ON prd.department_id = dep.id JOIN department_doctors as depd ON prd.department_doctor_id = depd.id JOIN doctor_schedules as ds ON prd.doctor_schedule_id = ds.id ';
    ...
    {
        $data = [];
        $data['item'] = DB::select('SELECT prd.id as id_data, prd.answare, prf.name, prf.type, p.id as pasien FROM `patient_registration_data` as prd JOIN patient_registration_forms as prf ON prd.patient_registration_form_id = prf.id JOIN patients as p ON prd.patient_id = p.id WHERE prd.kode_daftar = "' . $kode . '"');
